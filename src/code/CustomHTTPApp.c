...
//	MPFSGetFilename(curHTTP.file, filename, 20);
	...
	ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE*)"sendCmd");
	if(ptr)
	{
		// Validate the beginning of the command
		// If we find a bad command just do nothing 
		if(*ptr != '<'){}
		else if(*(ptr+1) != 'F'){}
		else if (*(ptr+2) != '1' && *(ptr+2) != '2' && *(ptr+2) != 'H'){}
		else 
		{
			int len;
			char device;
			device = (char)(*(ptr+2));			// device will equal '1', '2', or 'H'
			for (len=0; len<22; len++)
			{
				if(*(ptr+len) == '>')			// We found the end of the command!
				{
					char errorMsg[100] = {0};
					len-=4;						// don't care about the < F 1 and > characters
					focusingRotatorCommand newCmd;
					newCmd.source = WIRED_HTTP;
					InterpretCommandString((char*)(ptr+3), len, &newCmd, errorMsg);
					AddCmdToBfr(device, &newCmd );
					break;
				}	
			}	
		}	
...
HTTP_IO_RESULT HTTPExecutePost(void)
{
	...
	if(!memcmppgm2ram(filename, "focuser1.htm", 9))
	{
		int i = 0;
		for(i=0; i<20;i++);
	}	
...
{
	TCPPutString(sktHTTP, (BYTE*)&MyFocuser1.configStr.DeviceNickname);
	return;
}	
...
{
	BYTE pos[7] = {0};
	uitoa(MyFocuser1.statusStr.CurrentPosition, pos);
	TCPPutString(sktHTTP, pos);
	return;
}	
...
{
	BYTE temp[6] = {0};
	getTempString(MyFocuser1.statusStr.CurrentTemp, (char *)temp);
...
{
	BYTE temp[2] = {0};
	temp[0] = (MyFocuser1.tempCompInfoStr.TempCompOn + '0');
	TCPPutString(sktHTTP, temp);
	return;
}
